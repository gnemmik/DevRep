...
public class Actions {
	 ...
	 * empty or not, second we want to know if the text was saved or not. If the
	 * text area isn't empty & the text wasn't saved before this time, then the
	 * program display -> for the user an option for saving the text in a new file
	 * or in the same file
	 */
	public void neW() {
		/**
		 * if the text area isn't empty & if the text area hasn't a text not saved
		...
		if (!n.getTextArea().getText().equals("") && !n.getTextArea().getText().equals(fileContent)) {
			...
			if (fileName == null) {
				/**
				 * this method has 3 options (1 = YES, 2 = NO, 3 = Cancel) this is an option pop
				 * up to the user, for saving the old text or not
				 */
			...
			// if there was a text which was be opend or saved
				...
				option = JOptionPane.showConfirmDialog(null, "Do you want to save the changes ??");
				/**
				 * if the user click on YES button, then the program will save the text into the
				 * old file & make a new text area,,,
				 */
				if (option == 0) {
					save();
					// to create new getTextArea() after saving the old getTextArea()
		...
		/**
		 * if the text was be opened or saved before but wasn't be changed, and we want
		 * to write a new text,,, this option will be actived
		 */
		else {
			n.getTextArea().setText("");
		}
	...
	/**
	 * If we want to open a new text, first we want to know if the text area is
	 * empty or not, second we want to kwnow if the text was saved or not. If the
	 * text area isn't empty & the text wasn't saved befor this time, then the
	 * program display for the user an option for saving the text in a new file or
	 * in the same file
	 */
	public void opeN() {
		/**
		 * if the text area isn't empty & if the text area hasn't a text which not saved
		 * befor (fileContent != null)
		 */
		if (!n.getTextArea().getText().equals("") && !n.getTextArea().getText().equals(fileContent)) {
			/**
			...
			if (fileName == null) {
				...
				/**
				 * if the user click on NO button, the program will be opened the new documents
				 */
				if (option == 1) {
					// for opening the new documents
					open();
			...
			// if there was a text which was be opend or saved
				...
				/**
				 * if the user click on YES button, then the program will save the text into the
				 * same file & open the new documents
				 */
				if (option == 0) {
		...
		/**
		 * if the text was be opened or saved before but wasn't be changed, and we want
		 * to open a new document,,, this option will be actived
		 */
		else {
			// for opening the new documents
			open();
	...
	/**
	 * THIS IS FOR SAVE ACTION, SaveAs ACTION has saveAs() method If we want to save
	 * a new text, then we want to know if the text was saved befor or not. If the
	 * text wasn't be saved befor, then we will use saveAs() If the text was be save
	 * befor, then we will use save()
	 */
	public void savE() {
		/**
		 * if String fileName is null, then using saveAs(). Because there was no file
		 * was be saved
		 */
		if (fileName == null) {
			saveAs();
		...
		/**
		 * if String fileName has a String (file name), then using save(). Because we
		 * want to save the new text in the same file. if the user want to save the all
		 * text (old & new text) in a new file, -> he can presses SaveAs button
		 * (@saveAs())
		 */
		else {
			save();
	...
	/**
	 * THIS FROM SUN� WEBSITE (@Print.java) if we want to print the text, we can do
	...
	/**
	 * If we want to exit from the program, first we want to know if the text area
	 * is empty or not, second we want to kwnow if the text was saved or not. If the
	 * text area isn't empty & the text wasn't saved befor this time, then the
	 * program display -> for the user an option for saving the text in a new file
	 * or in the same file
	 */
	public void exiT() {
		...
		if (!n.getTextArea().getText().equals("") && !n.getTextArea().getText().equals(fileContent)) {
			...
			if (fileName == null) {
				...
				option = JOptionPane.showConfirmDialog(null, "Do you want to save the changes ??");
				/**
				 * if the user click on YES button, then the program will be saved the text &
				 * close the program
				 */
				if (option == 0) {
				...
				/**
				 * if the user click on NO button, then the program will be closed without save
				 * the text
				 */
				if (option == 1) {
					// for closing the program
		...
		/**
		 * if the text was be opened or saved before but wasn't be changed, and we want
		 * to close the program. This option will be actived
		 */
		else {
			// for closing the program
			System.exit(0);
	...
	public void lineWraP() {
		if (n.getLineWrap().isSelected()) {
			/**
			 * make the line wrap & wrap style word is true when the line wrap is selected
			 */
			n.getTextArea().setLineWrap(true);
			n.getTextArea().setWrapStyleWord(true);
		} else {
			/**
			 * make the line wrap & wrap style word is false when the line wrap isn't
			 * selected
			 */
	...
	/**
	 * @see FONTS.JAVA this is a font class which is for changing the font, style &
	 *      size
	 */
	public void fonT() {
	...
	/**
	 * @see ABOUT.JAVA it's a Message Dialog to show the information about this
	 *      program
	 */
	public void abouT() {
	...
	public void finD() {
		try {
			...
			while (n.getTextArea().getText().indexOf(findword) == -1) {
				/**
				 * this is a message dialog which is warning the user, because he didn't or
				 * forgot to enter the word
				 */
